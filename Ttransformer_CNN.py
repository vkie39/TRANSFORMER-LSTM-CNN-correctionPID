...
# 훈련 데이터와 테스트 데이터 분리
X_train, X_test, y_train, y_test = train_test_split(X_seq, y_seq, test_size=0.2, shuflle=False)
...
class TransformerBlock(layers.Layer):
    ...
    def call(self, inputs, training):
        ...
        attn_output = self.dropout1(attn_output, training=training)  # 위 결과에 대한 Dropout수행
        ...
        ffn_output = self.dropout2(ffn_output, training=training) # 위 결과에 Dropout적용
...
class CNNSelfAttentionModel(tf.keras.Model):
    def __init__(self, seq_length, d_model, num_heads, ff_dim, cnn_filters):
        ...
        self.conv1d = layers.Conv1D(filters=cnn_filters, kernel_size=3, activation="relu", padding="same") # 출력 데이터와 입력 데이터의 길이를 동일하게 유지함
        self.transformer = TransformerBlock(d_model, num_heads, ff_dim)
        self.global_pool = layers.GlobalAveragePooling1D()
        self.fc = layers.Dense(1, activation="linear")  # 회귀를 위한 출력층. fc 레이어
...
predicted_pid_value = predict_real_time(new_input)
print("Predicted PID Value:", predicted_pid_value)
