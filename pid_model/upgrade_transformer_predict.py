...
kp_scaler = joblib.load(kp_scaler_path)
ki_scaler = joblib.load(ki_scaler_path)
kd_scaler = joblib.load(kd_scaler_path)
...
class RealTimePredictor:
    def __init__(self, model, input_scaler, target_scaler, seq_length=5, feature_window=25, threshold1=0.3, threshold2=0.3, history_window=5):
        ...
        """
        self.model = model
        self.input_scaler = input_scaler  # 입력값 스케일러
        self.kp_scaler = kp_scaler        # 독립적인 `kp` 스케일러
        self.ki_scaler = ki_scaler        # 독립적인 `ki` 스케일러
        self.kd_scaler = kd_scaler        # 독립적인 `kd` 스케일러
        self.seq_length = seq_length     # 시퀀스 길이
        self.recent_inputs = []          # 최근 입력값 저장 리스트
    ...
    def predict_target(self):
        ...
        # 역스케일링 (self를 사용해 각 스케일러에 접근)
        prediction_kp = self.kp_scaler.inverse_transform(scaled_prediction[:, 0].reshape(-1, 1))
        prediction_ki = self.ki_scaler.inverse_transform(scaled_prediction[:, 1].reshape(-1, 1))
        prediction_kd = self.kd_scaler.inverse_transform(scaled_prediction[:, 2].reshape(-1, 1))

        # kd 값을 제한
        if prediction_kd[0, 0] > 0.6:  # kd 값이 0.6을 초과하면 제한
            print(f"kd value {prediction_kd[0, 0]} exceeds 0.6. Adjusting to 0.6.")
            prediction_kd[0, 0] = 0.6

        # 역스케일링된 결과를 합침
        prediction = np.hstack((prediction_kp, prediction_ki, prediction_kd)).flatten()
